...
def ReadPRISM(rgiYears,        # array containing the years that should be read
    ...
    ncid=Dataset('PRISM_daily_ppt_2017.nc', mode='r') # open the netcdf file
    ...
    for yy in range(len(rgiYears)):
        ...
        ncid=Dataset('PRISM_daily_ppt_'+str(rgiYears[0]+yy)+'.nc', mode='r') # open the netcdf file
...
def ReadERAI(grWTregion,
    ...
    ncid=Dataset('NewEngland_MFL850_daymean_2018.nc', mode='r') # open the netcdf file
...
def PreprocessWTdata(DailyVars,                      # WT data [time,lat,lon,var]
    ...
    # Normalize the data
    if NormalizeData == 1:
        dvmean = np.mean(DailyVars, axis=(1,2))[:,None,None,:]
        dvstd = np.std(DailyVars, axis=(1,2))[:,None,None,:]
        DailyVars=(DailyVars-dvmean)/dvstd
        # if ReferencePer is None:
...
def ClusterAnalysis(rgrWTdata,
                    ...
                    Plot=0,  
                    numWT=0):
                    # 0 - no plots; 1 - plots will be saved to sPlotDir
                      # 0 - use Hierarchical Cluster Analysis to determine number of weather types
    ...
    if numWT == 0:
        ...
        if Plot == 1:
            ...
            sPlotFile=sPlotDir
            sPlotName= 'BottomUp-Hirarch-Cluster_Dendrogram_' + plotnamemod + '_'+YYYY_stamp+'.pdf'
            ...
            sPlotFile=sPlotDir
            sPlotName= 'Accelaration-Of-Distance-Growth_'+plotnamemod+'_'+YYYY_stamp+'.pdf'
            if os.path.isdir(sPlotFile) != 1:
                subprocess.call(["mkdir","-p",sPlotFile])
            print('        Plot map to: '+sPlotFile+sPlotName)
            fig.savefig(sPlotFile+sPlotName)
        ...
        if rgiClusterMembers.max() >= 2:
            rgrClustersFin=kmeans2(rgrDataCluster,rgrClusters)
        else:
            rgrClustersFin=kmeans2(rgrDataCluster,2,minit='points')
    else:
        rgrClustersFin=kmeans2(rgrDataCluster,numWT,minit='points')
...
def XWT(training_predictors, # predictor variables that are used to train the model
    ...
    training_predictant,     # predictant variable that is used to train the model
    testing_predictant,      # predictant variable that is used to evaluate the model
