...
# for i in {0..99}; do
    ...
    sed -i "s/^    # call_compute_rmse_map_GARDLENS()/    call_compute_rmse_map_GARDLENS()/" $file_name
...
# for i in {0..99}; do
    # Create a filename based on parameters
    batch_name="gardlens_rmse_batch_${i}.pbs"

            # Copy content from the template script to a new file
    cp "template_WT_function_calls_batch.pbs" "$batch_name"

    sed -i "s/0_func_call/${i}_gardlens_rmse/" $batch_name
    sed -i "s/func_call_0.py/gardlens_rmse_${i}.py/" $batch_name
